...
-- Instagram Follower Tracker Database Schema
...
-- Follower counts table to track follower count over time
...
-- Grant permissions (optional, Supabase handles this automatically)
-- But we'll add comments for documentation
COMMENT ON TABLE whitelist IS 'Users excluded from analysis (e.g., celebrities, close friends)';
COMMENT ON TABLE non_followers IS 'Users who do not follow back';
COMMENT ON TABLE ex_followers IS 'Users who used to follow but stopped';
COMMENT ON TABLE users IS 'Application users for authentication';
COMMENT ON TABLE follower_counts IS 'Historical follower count tracking';
